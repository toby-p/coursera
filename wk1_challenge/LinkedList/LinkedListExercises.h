...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // If new data is less than head, make it the new head:
  Node* temp = head_;
  if (newNode->data < temp->data) {
    ...
    // Keep updating temp to the next node until it reaches  
    // tail or a lesser value:
    while (temp != tail_) {
    ...
    if (temp == tail_) {
      if (temp->data < newNode->data) {
        // newNode should be new tail.
        temp->next = newNode;
        newNode->prev = temp;
        tail_ = newNode;
      }
      else {
        // newNode should be the last node before tail
      ...
      // newNode is somewhere in the middle of the list
      temp->prev->next = newNode;
      newNode->prev = temp->prev;
      newNode->next = temp;
      temp->prev = newNode;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  while (left.size_ > 0 || right.size_ > 0) {
    if (left.size_ > 0 && right.size_ > 0) {
      int left_item = left.front();
      int right_item = right.front();
      if (left_item < right_item) {
        merged.pushBack(left_item);
        left.popFront();
      } else {
        merged.pushBack(right_item);
        right.popFront();
      }
    }
